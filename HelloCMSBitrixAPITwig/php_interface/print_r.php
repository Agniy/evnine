...
class dBug {
	...
	function makeTableHeader($type,$header,$colspan=2,$depth) {
		...
		if ($header!=='Array'){
			echo "<tr>";
			echo "<td ".$str_i."class=\"dBug_".$type."Header\" colspan=".$colspan." onClick='dBug_toggleTable(this)'>".$header."</td>
				</tr>";
			}
	}
	...
	function makeTDHeader($type,$header) {
		...
		if ($type==='array'){
				if (preg_match("/\:/",$header,$tmp)){
					$header = preg_replace("/:/","&nbsp;&nbsp;&nbsp;</br>Method: ",$header);
					$header = 'Class: '.$header;
				}
		}elseif($header[0]==='+'||$header[0]==='-'||$header[0]==='#'
		) {
			$header = substr($header,1,strlen($header));
		}
		echo "<tr".$str_d.">
				<td valign=\"top\" onClick='dBug_toggleRow(this)' class=\"dBug_".$type."Key\">".$header."</td>
				<td  class=\"dBug_".$type."KeyBody\">";
	}
	...
	//check variable type
	function checkType($var,$type,$depth) {
		switch(gettype($var)) {
			case "resource":
			...
			case "object":
				...
				break;
			case "array":
				$this->varIsArray($var,$type,$depth);
				break;
			case "NULL":
				$this->varIsNULL();
				break;
	...
	function varIsArray($var,$type='array',$depth) {
		$var_ser = serialize($var);
		array_push($this->arrHistory, $var_ser);
			if (empty($type)) 
				$type='array';
			$first_key = $this->getFirstArrayKey($var);
			if ($first_key[0]=='+')
				$type='arrayNew';
			if ($first_key[0]=='-')
				$type='arrayOld';
		...
		$this->makeTableHeader($type,'Array');
		if(is_array($var)) {
...
//			$type='array';
	...
	function varIsObject($var) {
		...
		if(is_object($var)) {
			...
			foreach($arrObjVars as $key=>$value) {
				...
				$value=(!is_object($value) && !is_array($value) && trim($value)=="") ? "[empty]" : $value;
	...
	function initJSandCSS() {
		echo <<<SCRIPTS
			...
			<style type="text/css">
				pre{font-family:12px Arial!important;white-space:pre-wrap!important;white-space:-moz-pre-wrap!important;white-space:-o-pre-wrap!important;white-space:-pre-wrap!important;word-wrap:break-word}
				table.dBug_arrayNew,table.dBug_arrayOld,table.dBug_array,table.dBug_object,table.dBug_resource,table.dBug_resourceC,table.dBug_xml {
					font-family:Verdana, Arial, Helvetica, sans-serif !important; color:#000000 !important; font-size:12px !important;
				}
				...
				.dBug_xmlHeader 
					{ font-weight:bold !important; color:#FFFFFF !important; cursor:pointer !important; }
				...
				/* array */
				table.dBug_array { background-color:#888888 !important;position:relative !important;z-index:1000}
				table.dBug_arrayNew { background-color:#888888 !important;}
				table.dBug_arrayOld { background-color:darkgray !important; }
				table.dBug_array td { background-color:#FFFFFF !important; }
				table.dBug_array td.dBug_arrayHeader { background-color:#AAAAAA !important;color:white !important;}
				table.dBug_array td.dBug_arrayNewHeader{ background-color:#cfc !important;color:green !important; } 
				table.dBug_array td.dBug_arrayOldHeader{ background-color:lightgray !important;color:gray !important; } 
				table.dBug_array td.dBug_arrayKey { background-color:#DDDDDD !important; }
				table.dBug_array td.dBug_arrayNewKey { background-color:#cfc !important;color:green !important; }
				table.dBug_array td.dBug_arrayOldKey { background-color:lightgray !important;color:gray !important; }
				table.dBug_array td.dBug_arrayNewKeyBody { color:green !important;padding:1px !important; }
				table.dBug_array td.dBug_arrayOldKeyBody { color:#808080 !important; }

				table.dBug_arrayBug { background-color:#884488  }
				table.dBug_array td.dBug_arrayBugHeader{ background-color:#AA66AA !important;color:white !important; } 
				table.dBug_array td.dBug_arrayBugKey { background-color:#FFDDFF !important;color:black !important; }
				table.dBug_array td.dBug_arrayBugKeyBody { color:black !important;padding:1px !important; }

				
				/* object */
				table.dBug_object { background-color:#0000CC !important; }
				table.dBug_object td { background-color:#FFFFFF !important; }
				...
				/* resource */
				table.dBug_resourceC { background-color:#884488 !important; }
				table.dBug_resourceC td { background-color:#FFFFFF !important; }
				table.dBug_resourceC td.dBug_resourceCHeader { background-color:#AA66AA !important; }
				table.dBug_resourceC td.dBug_resourceCKey { background-color:#FFDDFF !important; }
				
				/* resource */
				table.dBug_resource { background-color:#884488 !important; }
				table.dBug_resource td { background-color:#FFFFFF !important; }
				table.dBug_resource td.dBug_resourceHeader { background-color:#AA66AA !important; }
				table.dBug_resource td.dBug_resourceKey { background-color:#FFDDFF !important; }
				
				/* xml */
				table.dBug_xml { background-color:#888888 !important; }
				table.dBug_xml td { background-color:#FFFFFF !important; }
				table.dBug_xml td.dBug_xmlHeader { background-color:#AAAAAA !important; }
				table.dBug_xml td.dBug_xmlKey { background-color:#DDDDDD !important; }
			</style>
SCRIPTS;
	}
